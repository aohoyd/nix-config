...
let
  ...
  inherit (builtins) elem readDir match pathExists attrValues;
in rec {
  ...
  mapModulesExclude = exclude: path: fn:
    let apply = fn: path: n: fn (path + ("/" + n));
        ...
        notInList = list: n: !(elem n list);
        named = n: x: nameValuePair ((removeSuffix ".nix") n) x;
    in
      listToAttrs (map
        (n: named n (apply fn path n))
        (filter (isModuleIn path) (filter (notInList exclude) (attrsIn path))));
  mapModules = path: fn:
    mapModulesExclude [] path fn;
}